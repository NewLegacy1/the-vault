...
export interface GhostAutopsyReport {
  ...
  totalNetR: number;
  /** net R > 0, n ≥ 3, wins ≥ 2 — candidate to relax filter */
  relaxCandidates: GhostReasonRow[];
  /** net R ≤ 0 and n ≥ 3 — filter is earning its keep */
  keepStrict: GhostReasonRow[];
  /** Small sample — interesting but not actionable alone */
...
const PINE_HINTS: Record<number, { input: string; note: string }> = {
...
/** Parse pasted table: reason | n | W/L/scr | netR per line (flexible separators). */
export function parseGhostAutopsyPaste(text: string): GhostReasonRow[] {
  ...
  for (const line of lines) {
    ...
    if (/^\d+$/.test(parts[0]) && parts.length >= 3) {
      ...
      reason = GHOST_REASON_LABELS[id] ?? parts[1];
      nIdx = 2;
      wlIdx = 3;
      rIdx = 4;
    }
...
export function analyzeGhostAutopsy(rows: GhostReasonRow[], fullPaste = ""): GhostAutopsyReport {
  ...
  if (rows.length === 0) {
    recommendations.push("Paste rows from the Pine MISSED table (bottom right) after your replay finishes.");
  }
