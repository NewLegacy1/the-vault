...
- **Data requirement:** Needs **tick / volume-at-price / depth**. TradingView alone is **weak** for reliable absorption reads (no native Numbers Bars / true bid×ask VAP on typical prop TV charts). **Sierra Chart Numbers Bars** (native footprint) + depth is the serious path.
